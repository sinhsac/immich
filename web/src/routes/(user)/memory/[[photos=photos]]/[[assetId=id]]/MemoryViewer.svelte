<script lang="ts">
  ...
  // FORK: Import extension metadata service for sub-category display
    ...
    <ControlAppBar onClose={() => goto(Route.photos())} forceDark multiRow>
      {#snippet leading()}
        {#if current}
          ...
          <!-- FORK: Display sub-category badge when enriched metadata provides one -->
          {#if getCachedMemoryMetadata(current.memory.id)?.subCategory}
            <span class="ms-2 rounded-full bg-white/20 px-2 py-0.5 text-xs text-gray-200">
              {getCachedMemoryMetadata(current.memory.id)?.subCategory}
            </span>
          {/if}
        {/if}
      {/snippet}
          ...
          >
            ...
            {#if current.previousMemory}
              <div class="absolute inset-e-4 bottom-4 text-start text-white">
                ...
                <!-- FORK: Display sub-category badge for previous memory when available -->
                {#if getCachedMemoryMetadata(current.previousMemory.id)?.subCategory}
                  <span class="mt-1 inline-block rounded-full bg-white/20 px-2 py-0.5 text-xs text-gray-200">
                    {getCachedMemoryMetadata(current.previousMemory.id)?.subCategory}
                  </span>
                {/if}
              </div>
            {/if}
          ...
          >
            ...
            {#if current.nextMemory}
              <div class="absolute inset-s-4 bottom-4 text-start text-white">
                ...
                <!-- FORK: Display sub-category badge for next memory when available -->
                {#if getCachedMemoryMetadata(current.nextMemory.id)?.subCategory}
                  <span class="mt-1 inline-block rounded-full bg-white/20 px-2 py-0.5 text-xs text-gray-200">
                    {getCachedMemoryMetadata(current.nextMemory.id)?.subCategory}
                  </span>
                {/if}
              </div>
            {/if}
